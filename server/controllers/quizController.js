...
export const createQuiz = async (req, res) => {
  try {
    ...
    const quiz = new Quiz({ title, questions, isPublished, userId: req.user.id });
...
export const getAllQuizzes = async (req, res) => {
  try {
    const quizzes = await Quiz.find({ isPublished: true }).populate("userId", "name email profilePicture");
    res.status(200).json(quizzes);
  } catch (error) {
